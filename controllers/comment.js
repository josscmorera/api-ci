...
const createComment = async (req, res) => {
  try {
    ...
    const comment = await Comment.findById(savedComment._id).populate("user");
...
const getComments = async (req, res) => {
  try {
    ...
    const { post, parent, user } = req.query;
    ...
    if (user) {
      comments = await Comment.find({ user })
        .sort({ createAt: -1 })
        .populate("user")
        .populate("post");
      return res.status(200).json({ success: true, data: comments });
    ...
    comments = await Comment.find().sort({ createAt: -1 }).populate("user");
